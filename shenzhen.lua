...
function getAnimParams(cardParams)
	...
	local y0 = cardParams[3]*6 + 30 -- cardnum
	local x1 = cardParams[2]*18 + 30 -- pile1n
	local y1 = 10
	local xd = (x0 - x1) / 8
	local yd = (y0 - y1) / 8
...
function animateCards()
	if firstAnim == 0 then 
		...
		animCard = piles[params[1]][params[3]]
		table.remove(piles[params[1]], params[3])
...
end-- title:  Shenzhen I/O Solitaire
-- author: Gecko05
-- desc:   Solitaire from Shenzhen I/O
-- script: lua
-- input:  mouse
...
function update() 
	...
	if hold == false and md then
		...
		if my > 10 and my < 34 and mx > 103 and mx < 110 then
			btnNum = (my - 10)//8 + 1
